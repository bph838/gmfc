...
export function renderDriver(
  ...
  driver: { name: string; fastestLap: number; fastestLapTime: string | number | Date; averageLap: number; lapCount: number; },
  position: number,
  ...
  stat_laps_value.innerHTML = String(driver.lapCount);
  if (driverInformation) {   
    ...
    driverInformation.build?.forEach((text: string | null | undefined) => {
...
export function toggleDriverInfo(driverElement: { querySelector: (arg0: string) => any; }) {

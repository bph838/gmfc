    ...
    if (data) {
      ...
      if (wind_widget) {
        ...
        if (windDir > 90 && windDir < 270) {
          wind_widget.style.setProperty(
            "margin-top",
            `-${wind_widget_size / 6}px`,
          );
        }
      }
...
function getMPH(kmh: number) {
  return Math.round(kmh * 0.621371);
}

export function getWeatherImageAndLabel(
...
export function renderWindWidget(
  ...
  shaft.setAttribute("y1", String(center));
  shaft.setAttribute("x2", String(center));
  shaft.setAttribute("y2", String(edge + arrowHeight)); // leave space for arrow
...
function dirctionToCompass(deg: number) {
  const directions = [
    "North",
    "North East",
    "East",
...
function fetchIsNight(daylightData: any[]) {

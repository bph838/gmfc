...
let external_Path: string | null = null;

export function setGalleryData(data: any[], externalPath: string) {
  Loaded_Gallery_Data = data;
  external_Path = externalPath;
}

export function getAreVideosIncluded() {
  if (Loaded_Gallery_Data && Array.isArray(Loaded_Gallery_Data)) {
    return Loaded_Gallery_Data.some(
      (galleryItem) => "youtubeurl" in galleryItem,
    );
  }
  return false;
}
...
export function renderGallery(sections: HTMLElement, type: string, displayDates = "none") {
  ...
  if (displayDates == "years" || displayDates == "months") {
    timePeriodSections.forEach((yearDivId) => {
...
function checkGalleryYearDiv(parent: HTMLElement, date: string) {
  ...
  if (!yearDiv) {
    ...
    timePeriodSections.push(yearDiv.id);
  }
  return yearDiv;
}

function checkGalleryMonthDiv(parent: HTMLElement, date: string) {
  let dateObj = new Date(date.replace(" ", "T"));
  let month = dateObj.getMonth();
  let monthName = dateObj.toLocaleString("default", { month: "long" }); // "January"

  let monthDiv = document.getElementById(`gallery-year-${month}`);
  if (!monthDiv) {
    const monthHolderDiv = createDiv(parent, "gallery-year-section");

    const moenthHeader = createDiv(monthHolderDiv, "gallery-year-header");
    createH3(moenthHeader, monthName);

    let monthId = `gallery-year-${month}`;
    monthDiv = createDiv(monthHolderDiv, "gallery-year-items", monthId);
    timePeriodSections.push(monthDiv.id);
  }
  return monthDiv;
}
...
displayDates = "years","months","none"
*/
function renderGalleryImage(image: any, galleryDiv: HTMLElement, displayDates = "none") {
  ...
  if (displayDates == "years") {
    timePeriodDiv = checkGalleryYearDiv(galleryDiv, image.date);
  } else if (displayDates == "months") {
    timePeriodDiv = checkGalleryMonthDiv(galleryDiv, image.date);
  }

  const alink = createLink(timePeriodDiv, imgPath);
...
function renderGalleryVideo(video: any, galleryDiv: HTMLElement, displayDates = "none") {
  ...
  let timePeriodDiv = galleryDiv;
  if (displayDates == "years") {
    timePeriodDiv = checkGalleryYearDiv(galleryDiv, video.date);
  } else if (displayDates == "months") {
    timePeriodDiv = checkGalleryMonthDiv(galleryDiv, video.date);
  }
  ...
  let innerDiv = createDiv(timePeriodDiv, "gallery_video_holder");

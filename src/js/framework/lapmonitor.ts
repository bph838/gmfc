 ...
 */
export async function fetchLapData(url: RequestInfo | URL) {
 ...
 * @returns {string}
 */
export function formatLapTime(hundreds: number) {
  const seconds = hundreds / 100;
  return `${String(seconds).padStart(2, "0")}`;
}

function formatLapDateTime(at: Date) {
  return formatDate(at);
}

type RawLap = { d: number; t: string | number };
type RawDriver = {
  name: string;
 ...
 */
export function processLeaderboard(rawData: Record<string, RawDriver>) {
 ...
 */
export async function loadLeaderboard(url: any) {

...
class ExcelToCsvAndJsonPlugin {
  ...
  apply(compiler) {
    compiler.hooks.afterCompile.tapAsync('ExcelToCsvAndJsonPlugin', (compilation, callback) => {
      const { input, sheetName, csvOutput, jsonOutput } = this.options;
      ...
      try {
        ...
        if (!workbook.SheetNames.includes(sheetName)) {
          compilation.errors.push(new Error(`Sheet "${sheetName}" not found in ${input}`));
          return callback();
        }

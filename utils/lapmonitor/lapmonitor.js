...
const FILE_DRIVERS = "..\\..\\src\\lapmonitor\\drivers\\drivers.json";
const DIR_RAW_FILES_REL = "..\\..\\src\\lapmonitor\\rawfiles";
const DIR_RAW_FILES_ABS =
  "D:\\Gordano Model Flying Club\\gmfc\\src\\lapmonitor\\rawfiles\\";
const FILE_OUTPUT = "..\\..\\.build\\lapmonitor\\driver_laps.json";
const DRIVER_OUTPUT = "..\\..\\.build\\lapmonitor";

async function loadDrivers() {
  const driverFile = path.join(__dirname, FILE_DRIVERS);
...
async function processLapFiles() {
  const dir = path.join(__dirname, DIR_RAW_FILES_REL);
...
async function saveResults() {
  ...
  await fs.mkdir(path.join(__dirname, DRIVER_OUTPUT), { recursive: true });
  ...
  for (const [driver, data] of driverLaps.entries()) {
    ...
    const outDriverFile = path.join(
      __dirname,
      `${DRIVER_OUTPUT}\\${driver}.json`,
    );
    await fs.writeFile(outDriverFile, JSON.stringify(data, null, 2));
  }

  const outFile = path.join(__dirname, FILE_OUTPUT);
...
async function saveSummary() {
  ...
  let driverData = [];
  let driverSummary = {};
  const inFile = path.join(__dirname, FILE_OUTPUT);
...
async function run() {
  ...
  await renameLapMonitorFiles(DIR_RAW_FILES_ABS);
  await loadDrivers();
